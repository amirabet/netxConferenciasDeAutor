...
if($_POST){
	...
	if($robotest){
		$error_robotest = "<em>Nice try robot, nice try...</em>";
	}else{ //No honeypot
		//if($from_name && $from_email && $from_telf){ //Camps omplerts
		if($from_name && $from_email){ //Camps omplerts
			...
			if($valid_name == 2 && $valid_email == 2 && $valid_telf == 2){ //Camps Correctes
				...
				if(!$exito){ // FALLO EN EL ENVIO DEL MENSAJE
					...
					$error_smpt .= '<b>' . $company_mail . '</b><br/></p>';
					$error_smpt .= '<p class="hidden">' . $mail->ErrorInfo . '</p>';
					// Test google img tracking on local
					$error_smpt .= '<img src="' . get_stylesheet_directory_uri() . '/images/layout/tracking_img.gif" alt="tracking send form ga" onload="_gaq.push([\'_trackEvent\', \'masinfo\', \'formulario\', \'fallo\']);"/>';
				}else{ //MENSAJE ENVIADO !!!		
					$success = '<p>Gracias por tu atención, <b>' . utf8_encode($from_name) . '</b>.';
					$success .= '<img src="' . get_stylesheet_directory_uri() . '/images/layout/tracking_img.gif" class"fr" alt="tracking send form ga" onload="_gaq.push([\'_trackEvent\', \'masinfo\', \'formulario\', \'enviado\']);"/>';
					...
					$success .= '<br/ >Correo Electrónico: <b>' . $from_email . '</b><br/></p>';
				...
				$not_valid = 'Por favor corrige los campos marcados en rojo.';
			}
		}else{ //Camps buits
...
<article id="form_wrap" class="wrap_readmore">
<?php 
//Retorn de Variables
//
//Nombre
...
if (!isset($success) && !isset($error_smpt)){
	if ( isset($robotest) || isset($empty_fields) || isset($not_valid) ){ 
		if ( isset($error_robotest)){$error_message = $error_robotest; $error_header = $header_robotest; $error_icon = $icon_robotest;}
		elseif ( isset($empty_fields)){$error_message = $empty_fields; $error_header = $header_empty_fields; $error_icon = $icon_empty_fields;}
		elseif ( isset($not_valid)){$error_message = $not_valid; $error_header = $header_not_valid;  $error_icon = $icon_not_valid;}
		echo '<header><div class="box box-error"><h4><span class="fa ' . $error_icon . '"></span>' . $error_header . '</h4><p>' . $error_message . '</p></div></header><section>';
	}else{// Header Form?>
	<header><h4><?php echo $header_form; ?></h4><p><?php echo $header_form2; ?></p><span class="readmore"><span class="readmore_button"><i class="fa fa-info"></i></span></span></header>
	...
	<?php } //end if form header
		...
		if ($testerror == 1){
			...
			echo '<article class="box box-success"><section>' . $success . '</section></article>';
			$error1 = '<p><span class="fa fa-exclamation-triangle"></span>&nbsp;&nbsp;<em>Por favor rellena todos los campos.</em></p>';
			echo '<article class="box box-error"><section>' . $error1 . '</section></article>';
			...
			echo '<article class="box box-error"><section>' . $error2 . '</section></article>';
			...
			echo '<article class="box box-error"><section>' . $not_valid . '</section></article>';
		}
		...
		$testconfig = '1'; 
		if ($testconfig == 1){
			echo 'SMTP dir: ' . $smtp_plugindir . ' | SMTP host: ' . $smtp_host . ' | SMTP Port: ' . $smtp_port . '<br/>';
			echo 'Editor ID: ' . $editor_id . '<br/>';
			echo 'Forms Mail: ' . $smtp_user . ' | Forms Pass: ' . $smtp_pass . ' | Company email: ' . $company_mail;
		}
		// Final Tests
		?>
		...
		<!-- <form action="#form_anchor" method="post" name="form" id="form" class="form_foot" onSubmit="_gaq.push(['_trackEvent', 'Formulario', 'Enviar'])"> -->
			<ul>
				...
				<li class="form_telf <?php if($_POST){ if ( empty($from_telf) || $valid_telf == 1 ){echo 'error';}elseif($valid_telf == 2){echo 'valid';}}?>">
					<label for="ftelf"><?php echo $telefono;
				...
				*/ ?>
				...
				<li class="form_bttn">
					<input name="send" type="submit" class="button" value="<?php echo $enviar; ?>"/>
					<span id="form_loader" class="hidden">
						<img src="<?php echo get_stylesheet_directory_uri(); ?>/images/layout/loader_smll.gif" alt="loading" />
					</span>
				</li>
			</ul>
...
<?php }else{ //If $success && $error2 
	if (isset($success)){
		...
		echo '<header><div class="box box-success"><h4><span class="fa ' . $success_icon . '"></span>' . $header_success . '</h4></div></header><section><div class="box box-success">' . $success . '</div></section>';
		//echo '<section><article class="box box-success">' . $success . '</article></section>';
		...
		echo '<header><div class="box box-error"><h4><span class="fa ' . $icon_error_smpt . '"></span>' . $header_error_smpt . '</div></header><section><div class="box box-error">' . $error_smpt . '</div></section>';
		//echo '<section><article class="box box-error">' . $error_smpt . '</article></section>';	
	}
} ?>
</article>
