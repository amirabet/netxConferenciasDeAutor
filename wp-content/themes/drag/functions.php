...
function childtheme_head_profile() {
	...
	if (is_front_page()) {
		...
		$title = get_the_title() . " / " . $categories[0]->cat_name . " | " . get_bloginfo('name');
            ...
            if (!empty ($metafield)){
				$description = $metafield . " / " . $categories[0]->cat_name . " | " . get_bloginfo('name');
			}elseif (!empty($post->post_excerpt)) {
				$description = get_the_excerpt() . " / " . $categories[0]->cat_name . " / " . get_bloginfo('name');} 
			else {
	...
	//Automàtic amb ACF a la pàgina de l'autor >> OJU amb l'ID!!
	$acf_admin_fb = get_field('facebook_admin', 'user_2');
	if ($acf_admin_fb!=''){
		$admin_fb = $acf_admin_fb;
	}else{ 
	//Manual
		$admin_fb ='124616974234023';
	}
	if (!empty ($admin_fb)){
		$content .="<!-- Facebook insights metatags ><!-->" . "\n";
		$content .='<meta property="fb:app_id" content="1398228117096999" />' . "\n";
		$content .="<!-- END Facebook insights ><!-->" . "\n";
	}
	// Twitter Summary Card *******************************************
	//Usuari de Twitter
	//Automàtic amb ACF a la pàgina de l'autor >> OJU amb l'ID!!
	$acf_user_tw = get_field('twitter', 'user_2');
	if ($acf_user_tw!=''){
		$user_tw = $acf_user_tw;
	}else{ 
	//Manual
	...
	//Automàtic amb ACF a la pàgina de l'autor >> OJU amb l'ID!!
	$acf_user_google = get_field('google+', 'user_2');
	if ($acf_user_google!=''){
		$url_gplus = $acf_user_google;
	}else{ 
	//Manual
		$url_gplus ="116959801615366824799";
	}
	if (!empty ($url_gplus)){
		$content .="<!-- Google Authorship ><!-->" . "\n";
		$content .="<link rel='author' href='https://plus.google.com/u/0/" . $url_gplus . "' />" . "\n";
	}
	return $content;
}
...
function childtheme_show_description() {
	...
	elseif (is_single()) {
            ...
            if (!empty ($metafield)){
				...
				$categories = get_the_category();
				$content .= get_the_excerpt() . " / " . $categories[0]->cat_name . " | " . get_bloginfo('name');} 
			else {
				$categories = get_the_category();
				$content .= get_the_title() . " / " . $categories[0]->cat_name . " | " . get_bloginfo('description') . " / " . get_bloginfo('name');} 
        }
	elseif (is_page()) {
			if( qtrans_getLanguage() == 'es' ){
			 	$metafield = get_field ("metadescription_es");
